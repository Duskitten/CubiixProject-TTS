...
func first_run():
	...
	DynBones_Dict = DynBones_Register.duplicate(true)
	#print(DynBones_Register)
	##This is just some initialization stuff
	await RePositioned
	for x in DynBones_Register.size():
		for y in DynBones_Register[x]["DynBone_Sets"]:
			var current_parent = Skeleton
			for z in DynBones_Register[x]["DynBone_Sets"][y].size():
				var current_bone = Skeleton.find_bone(DynBones_Register[x]["DynBone_Sets"][y][z])
				DynBones_Dict[x]["DynBone_Sets"][y][z] = current_bone
				DynBones_OldPos[x]["DynBone_Sets"][y][z] = (Skeleton.global_transform * Skeleton.get_bone_global_pose(current_bone)).origin
				DynBones_NodeTailsVal[x]["DynBone_Sets"][y][z] = Vector3.ZERO
				DynBones_CoreRot[x]["DynBone_Sets"][y][z] = Skeleton.get_bone_pose_rotation(current_bone)
				if Skeleton.get_bone_children(current_bone).size() > 0:
					DynBones_RealTails[x]["DynBone_Sets"][y][z] = (Skeleton.global_transform * Skeleton.get_bone_global_pose(Skeleton.get_bone_parent(current_bone))).affine_inverse() * ((Skeleton.global_transform * Skeleton.get_bone_global_pose(current_bone)) * Vector3(0,(Skeleton.global_transform * Skeleton.get_bone_global_pose(current_bone)).origin.distance_to((Skeleton.global_transform * Skeleton.get_bone_global_pose(Skeleton.get_bone_children(current_bone)[0])).origin),0))
				else:
					DynBones_RealTails[x]["DynBone_Sets"][y][z] = (Skeleton.global_transform * Skeleton.get_bone_global_pose(Skeleton.get_bone_parent(current_bone))).affine_inverse() * ((Skeleton.global_transform * Skeleton.get_bone_global_pose(current_bone)) * Vector3(0,.2,0))
	canrun = true
...
func _process_modification() -> void:
	if canrun:
		for x in DynBones_Register.size():
			for y in DynBones_Register[x]["DynBone_Sets"]:
				var settings = DynBones_Register[x]["DynBone_Settings"][y]
				var damp_ratio = log(settings["dampening"]) / (-settings["osc_ps"] * settings["damp_time"])
				for z in DynBones_Register[x]["DynBone_Sets"][y].size():
					var delta = olddelta
					var current_bone = DynBones_Dict[x]["DynBone_Sets"][y][z]
					...
					var realtail = DynBones_RealTails[x]["DynBone_Sets"][y][z]
					...
					var vel_o = DynBones_NodeTailsVal[x]["DynBone_Sets"][y][z]
					var pos_cur_o = DynBones_OldPos[x]["DynBone_Sets"][y][z]
					DynBones_NodeTailsVal[x]["DynBone_Sets"][y][z] += Vector3(a * vel_o.x + b  * (pos_tar.x - pos_cur_o.x),a * vel_o.y + b  * (pos_tar.y - pos_cur_o.y),a * vel_o.z + b  * (pos_tar.z - pos_cur_o.z))
					DynBones_OldPos[x]["DynBone_Sets"][y][z] += (delta*vel_o)
					
					var ray_a :Vector3 =  (realtail - BonePos.origin).normalized()
					var ray_b :Vector3 =  ((Skeleton.global_transform * BoneParent).affine_inverse() * DynBones_OldPos[x]["DynBone_Sets"][y][z] - BonePos.origin).normalized()
