...
func _ready() -> void:
	if OS.has_feature("Client"):
		...
		gclientmanager= load("res://Mods/G_ClientManager/Scripts/ClientManager.gd").new()
		gassetmanager.setup(self)
		gmodloader.setup(self)
		gclientmanager.setup(self)
		var clientcore = load("res://Mods/G_Core/Scenes/Client_Core.tscn").instantiate()
		$Scripts.add_child(clientcore)
		$Scripts.add_child(gclientmanager)
		...
		$Scripts.add_child(gservermanager)
